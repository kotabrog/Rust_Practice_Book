...
use std::{
    error::Error,
    io::{BufRead, Read, Seek, BufReader, SeekFrom},
    fs::File,
};
use regex::Regex;
...
pub struct ConfigArgs {
    #[arg(name = "FILE", required = true, help = "Input file(s)")]
    files: Vec<String>,
    #[arg(short = 'n', long, default_value = "10", conflicts_with = "bytes", help = "Number of lines")]
    lines: String,
    #[arg(short = 'c', long, help = "Number of bytes")]
    bytes: Option<String>,
...
pub fn get_args() -> MyResult<Config> {
    ...
    let bytes = args.bytes.as_ref().map(|b| parse_num(b)).transpose()?;
    Ok(Config {
        files: args.files,
        lines,
        bytes,
